...
#ISSUE                FECHA                AUTOR                DESCRIPCION
 ...
 #MDID-12              16/10/2020          EGS                   Se agrega funcion que lista codigo de proyecto
 ***************************************************************************/
...
BEGIN
    ...
    ***********************************/
    ...
    ELSIF (p_transaccion='PRO_PROHITCOD_SEL') THEN

        BEGIN
            --Sentencia de la consulta
            v_consulta:='SELECT
                         DISTINCT(prohit.codigo)
                        FROM pro.tproyecto_hito prohit
                        JOIN segu.tusuario usu1 ON usu1.id_usuario = prohit.id_usuario_reg
                        LEFT JOIN segu.tusuario usu2 ON usu2.id_usuario = prohit.id_usuario_mod
                        WHERE  ';
        ...
        /*********************************
         #TRANSACCION:  'PRO_PROHITCOD_CONT'
         #DESCRIPCION:    Conteo de registros
         #AUTOR:        egutierrez
         #FECHA:        16/10/2020 20:15:06
         #ISSUE:        #MDID-12
        ***********************************/

    ELSIF (p_transaccion='PRO_PROHITCOD_CONT') THEN

        BEGIN
            --Sentencia de la consulta de conteo de registros
            v_consulta:='SELECT COUNT( DISTINCT(prohit.codigo) )
                         FROM pro.tproyecto_hito prohit
                         JOIN segu.tusuario usu1 ON usu1.id_usuario = prohit.id_usuario_reg
                         LEFT JOIN segu.tusuario usu2 ON usu2.id_usuario = prohit.id_usuario_mod
                         WHERE ';
